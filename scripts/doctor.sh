...
DEV_ROOT="${1:-/Volumes/ExtSSD/Projects/Work}"
PASS=0
WARN=0
FAIL=0
...
if [ -f "${USER_HOME}/.claude/CLAUDE.md" ] && grep -Fq '@~/.agentic-dotnet/instructions/global.md' "${USER_HOME}/.claude/CLAUDE.md"; then pass 'Claude global instructions adapter'; else fail 'Claude global instructions adapter'; fi
...
else fail 'dotnet executable'; fi
